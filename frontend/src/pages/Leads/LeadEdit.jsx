...
const LeadEdit = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', companyName: '', jobTitle: '', city: '', state: '', country: '', linkedinUrl: '',
    source: '', campaignName: '', utmMedium: '', utmSource: '', utmCampaign: '',
    status: 'NEW', leadScore: 0, industry: '', companySize: '', annualRevenue: '', budget: '', timeline: '', painPoints: '',
    dealStage: 'PROSPECTING', probability: 0, nextFollowUp: '',
    notes: '', assignedToId: '', tags: '', gdprConsent: false
  });
    ...
    const fetchLead = async () => {
      try {
        ...
        setFormData({
          ...
          dealStage: lead.dealStage || 'PROSPECTING',
  ...
  if (fetching) {
    return <div className="p-6 text-slate-500 animate-pulse">Loading lead details...</div>;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in pb-10">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Edit Lead</h1>
          <p className="text-sm text-slate-500 mt-1">Update lead tracking information</p>
        </div>
        <button
      ...
      {error && <div className="mb-4 bg-red-50 border border-red-200 text-red-600 p-3 rounded-xl text-sm font-medium">⚠️ {error}</div>}
          ...
          <div className={activeTab === 'basic' ? 'block' : 'hidden'}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <input type="url" name="linkedinUrl" value={formData.linkedinUrl} onChange={handleChange} className="form-input" />
              </div>
          ...
          <div className={activeTab === 'qualification' ? 'block' : 'hidden'}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input type="text" name="industry" value={formData.industry} onChange={handleChange} className="form-input" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" name="annualRevenue" value={formData.annualRevenue} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Budget ($)</label>
                <input type="number" name="budget" value={formData.budget} onChange={handleChange} className="form-input" />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea name="painPoints" value={formData.painPoints} onChange={handleChange} rows={3} className="form-input resize-none" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">General Notes</label>
                <textarea name="notes" value={formData.notes} onChange={handleChange} rows={2} className="form-input resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className={activeTab === 'tracking' ? 'block' : 'hidden'}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <input type="text" name="campaignName" value={formData.campaignName} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">UTM Source</label>
                <input type="text" name="utmSource" value={formData.utmSource} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">UTM Medium</label>
                <input type="text" name="utmMedium" value={formData.utmMedium} onChange={handleChange} className="form-input" />
              </div>
              <div>
          ...
          <div className={activeTab === 'pipeline' ? 'block' : 'hidden'}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <input type="number" name="assignedToId" value={formData.assignedToId} onChange={handleChange} className="form-input" />
              </div>
            </div>
          </div>
