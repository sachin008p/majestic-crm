...
export default function Dashboard() {
  ...
  /* ---------- ALL metrics computed from real DB data ---------- */
  ...
  /* trends + sparklines computed from real dates (null if no dates) */
    ...
    <div className="space-y-6 pb-10 min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 bg-[var(--card)] border border-[var(--border)] p-5 rounded-2xl shadow-sm">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button onClick={fetchData}
            className="px-4 py-2 rounded-xl bg-[var(--primary)] text-white hover:opacity-90 transition flex items-center gap-2 text-sm font-semibold">
            ↻ Refresh
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {loading ? (
          ...
          <>
            <StatCard title="Total Leads" value={leads.length} icon="📈"
              iconBg="rgba(59,130,246,.12)" accent="#3b82f6"
              trend={leadTrend} spark={leadSpark} sparkColor="#3b82f6" />
            <StatCard title="Customers" value={customers.length} icon="👥"
              iconBg="rgba(124,58,237,.12)" accent="#7c3aed"
              trend={custTrend} spark={custSpark} sparkColor="#7c3aed" />
            <StatCard title="Deals Closed" value={closedWon} icon="✅"
              iconBg="rgba(34,197,94,.14)" accent="#16a34a"
              trend={wonTrend} spark={wonSpark} sparkColor="#16a34a" />
            <StatCard title="Pending Tasks" value={pendingTasks} icon="⏳"
              iconBg="rgba(245,158,11,.14)" accent="#d97706"
              trend={taskTrend} spark={taskSpark} sparkColor="#f59e0b" />
          </>
        )}
      </div>
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-5">
            <div>
              ...
              <p className="text-xs text-[var(--muted)]">
                {closedWon} won / {leads.length} total
              </p>
            </div>
          ...
          <div className="w-full h-3 rounded-full bg-[var(--border)] overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700"
              style={{ width: `${winRate}%`, background: "linear-gradient(90deg,#16a34a,#22c55e)" }} />
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-1">
            {leads.slice(0, 6).map((lead) => (
              <div key={lead.id}
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition">
                <div className="w-10 h-10 rounded-xl grid place-items-center text-white font-bold text-sm shrink-0"
                ...
                {lead.value != null && (
                  <span className="font-bold text-sm hidden sm:block">{inr(lead.value)}</span>
                )}
            ...
            {tasks.slice(0, 6).map((task) => (
              <div key={task.id}
                ...
                <div className="w-10 h-10 rounded-xl grid place-items-center text-white font-bold shrink-0"
                  style={{ background: task.status === "COMPLETED" ? "#16a34a" : "#f59e0b" }}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-[var(--muted)] truncate">
                    ...
                    {task.dueDate || task.due_date ? ` · ${task.dueDate || task.due_date}` : ""}
                  </p>
                </div>
